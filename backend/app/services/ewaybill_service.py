...
def _validate_lr_exists(db, lr_id: int) -> None:
...
def get_ewaybills_for_lr(lr_id: int) -> List[dict]:
    db = SessionLocal()
    try:
        rows = (
            db.query(EWayBillModel)
...
def get_ewaybill_by_id(eway_id: int) -> Optional[dict]:
    db = SessionLocal()
    try:
...
def update_ewaybill(eway_id: int, payload: dict) -> Optional[dict]:
    db = SessionLocal()
    try:
        obj = db.query(EWayBillModel).filter(EWayBillModel.id == eway_id).first()
        if not obj:
...
def extend_ewaybill(eway_id: int, new_valid_upto) -> Optional[dict]:
    db = SessionLocal()
    try:
        obj = db.query(EWayBillModel).filter(EWayBillModel.id == eway_id).first()
        if not obj:
